...
import { Cpu, Wifi, WifiOff, Camera, Activity, Plus, Loader2, ShieldOff, Thermometer, X, Trash2 } from 'lucide-react'
...
export default function Devices() {
  ...
  const handleDeleteDevice = async (deviceId) => {
    ...
    try {
      await dashboardService.deleteDevice(deviceId)
                  ...
                  <div className="flex items-center gap-3">
                    <div
                      className={`flex h-10 w-10 items-center justify-center rounded-xl ${
                        deviceTypeColors[device.device_type] || 'bg-surface-500/15'
                      }`}
                    >
                      <Icon
                        className={`h-5 w-5 ${
                          deviceTypeIconColors[device.device_type] || 'text-surface-400'
                        }`}
                      />
                    </div>
                    <div>
